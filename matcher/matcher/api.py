...
def do_match(jurisdiction, event_type):
    ...
    df = pd.concat([load_data_for_matching(jurisdiction, event_type, S3_BUCKET, keys) for event_type in EVENT_TYPES])
    ...
    for event_type in EVENT_TYPES:
        write_matched_data(df, jurisdiction, event_type, S3_BUCKET, PG_CONNECTION)
